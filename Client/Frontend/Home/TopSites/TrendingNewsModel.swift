...
class TrendingNewsModel {
    ...
    var trendingStory: StoryFeedModel?
...
extension TrendingNewsModel: HomepageSectionHandler {
    func configure(_ collectionView: UICollectionView,
        ...
        switch isLastItem {
        case true:
            guard let cell = collectionView.dequeueReusableCell(cellType: ViewRecentlyCell.self, for: indexPath)
            else {
                return UICollectionViewCell()
            }
            
            cell.closureTappedOnBtnAction = { [weak self] btn in
                self?.closureDidTapOnViewRecentlyCell?(btn)
            }
            
            cell.applyTheme(theme: self.theme)
            return cell
        case false:
            guard let cell = collectionView.dequeueReusableCell(cellType: TrendingItemCell.self, for: indexPath)
            else {
                return UICollectionViewCell()
            }
            if (self.trendingStory?.stories?.count ?? 0) > indexPath.row,
               let trendingModel = self.trendingStory?.stories?[indexPath.row] {
